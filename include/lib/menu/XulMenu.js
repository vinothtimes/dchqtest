...
{
   if (menu1.browser.ie && menu1.browser.ie5) {
       sel = document.body.getElementsByTagName("SELECT");
...
function XulMenu(id) {
    ...
    this.hide = function(id) {
        ...
        if (this.fixIeSelectBoxBug && this.browser.ie6) {
            var iframe = document.getElementById(id+"-section-iframe");
            if (iframe) {
    ...
    this.show = function(id) {
        ...
        if (this.fixIeSelectBoxBug && this.browser.ie6) {
            var div = document.getElementById(id+'-section');
            var iframe = document.getElementById(id+"-section-iframe");
            if (iframe) {
    ...
    function hideInit()
    {
        if (!self.showOnOver) return;
        hideCancel();
        self.hideId = setTimeout(hidePerform, self.hideDelay);
    }
    function hidePerform()
    {
        if (!self.showOnOver) return;
        self.hideAll();
        hideCancel();
