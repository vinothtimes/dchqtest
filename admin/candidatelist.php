...
JB_admin_header('Admin -> Candidate List');

?>

<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000; "></div>
<b>[Candidates List]</b><span style="background-color: <?php if (($_REQUEST['mode']!='EDIT')) { echo "#F2F2F2"; }  ?>; border-style:outset; padding: 5px;"><a href="signup_candidate_form.php?mode=VIEW">View Form</a></span> <span style="background-color:  <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']=='')) { echo "#FFFFCC"; }  ?>; border-style:outset; padding: 5px;"><a href="signup_candidate_form.php?mode=EDIT">Edit Fields</a></span> <span style="background-color: <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']!='')) { echo "#FFFFCC"; }  else { echo "#F2F2F2";}?> ; border-style:outset; padding: 5px;"><a href="signup_candidate_form.php?NEW_FIELD=YES&mode=EDIT">New Field</a></span> &nbsp; &nbsp; <span style="background-color: <?php  echo "#FFFFCC";?> ; border-style:outset; padding: 5px;"><a href="candidatelist.php">Candidate List</a></span>
...
if ($_REQUEST['save_col']!='') {
	...
	if ($_REQUEST['field_id']==false) {
		$error = "Did not select a field <br>";
	}
...
echo '<a href="candidatelist.php">+ Add new column</a>';

}

?>
	<form method="POST" action="candidatelist.php">
...
?>
	...
	<tr>
		...
		JB_field_select_option_list (5, $col_row['field_id']);
	...
	if ($_REQUEST['column_id']==false) { // get the last sort order

		$sql = "SELECT max(sort_order) FROM form_lists WHERE field_id=5 GROUP BY column_id ";
		$result = JB_mysql_query ($sql) or die (mysql_error().$sql);
