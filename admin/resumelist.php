...
if ($_REQUEST['save_col']!='') {
	...
	if ($field_row['template_tag']=='') { // need to fix the template tag!
		...
		$field_row['template_tag'] = JB_generate_template_tag(2);

		// update form field

		$sql = "UPDATE form_fields SET `template_tag`='".jb_escape_sql($field_row['template_tag'])."' WHERE form_id=2 AND field_id='".jb_escape_sql($_REQUEST['field_id'])."'";
		JB_mysql_query ($sql);
...
echo '<a href="resumelist.php">+ Add new column</a>';

}

?>
	<form method="POST" action="resumelist.php">
...
?>
	...
	<tr>
		...
		JB_field_select_option_list (2, $col_row['field_id']);
	...
	if ($_REQUEST['column_id']=='') { // get the last sort order
		...
		$sql = "SELECT max(sort_order) FROM form_lists WHERE field_id=2 GROUP BY column_id ";
		$result = JB_mysql_query ($sql) or die (mysql_error().$sql);
