...
JB_admin_header('Admin -> Employer List');


?>
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000; "></div>
<b>[Employers List]</b><span style="background-color: <?php if (($_REQUEST['mode']!='EDIT')) { echo "#F2F2F2"; }  ?>; border-style:outset; padding: 5px;"><a href="signup_employer_form.php?mode=VIEW">View Form</a></span> <span style="background-color:  <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']=='')) { echo "#FFFFCC"; }  ?>; border-style:outset; padding: 5px;"><a href="signup_employer_form.php?mode=EDIT">Edit Fields</a></span> <span style="background-color: <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']!='')) { echo "#FFFFCC"; }  else { echo "#F2F2F2";}?> ; border-style:outset; padding: 5px;"><a href="signup_employer_form.php?NEW_FIELD=YES&mode=EDIT">New Field</a></span> &nbsp; &nbsp; <span style="background-color: <?php  echo "#FFFFCC";?> ; border-style:outset; padding: 5px;"><a href="employerlist.php">Employers List</a></span>
...
if ($_REQUEST['save_col']!='') {
	...
	if (is_numeric($_REQUEST['field_id'])) {
		...
		$fields = JB_schema_get_static_fields(4);
	...
	if ($field_row['template_tag']=='') { // need to fix the template tag!
		...
		$field_row['template_tag'] = JB_generate_template_tag(4);

		// update form field

		$sql = "UPDATE form_fields SET `template_tag`='".jb_escape_sql($field_row['template_tag'])."' WHERE form_id=4 AND field_id='".jb_escape_sql($_REQUEST['field_id'])."'";
		JB_mysql_query ($sql);
...
echo '<a href="employerlist.php">+ Add new column</a>';
...
?>
	...
	<tr>
		...
		JB_field_select_option_list (4, $col_row['field_id']);
	...
	if ($_REQUEST['column_id']=='') { // get the last sort order
		...
		$sql = "SELECT max(sort_order) FROM form_lists WHERE field_id=4 GROUP BY column_id ";
		$result = JB_mysql_query ($sql) or die (mysql_error().$sql);
