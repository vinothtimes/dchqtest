...
JB_admin_header('Admin -> Edit Post List');

?>
<div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000; "></div>
<b>[Job List]</b><span style="background-color: <?php if (($_REQUEST['mode']!='EDIT')) { echo "#F2F2F2"; }  ?>; border-style:outset; padding: 5px;"><a href="postform.php?mode=VIEW">View Form</a></span> <span style="background-color:  <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']=='')) { echo "#FFFFCC"; }  ?>; border-style:outset; padding: 5px;"><a href="postform.php?mode=EDIT">Edit Fields</a></span> <span style="background-color: <?php if (($_REQUEST['mode']=='EDIT') && ($_REQUEST['NEW_FIELD']!='')) { echo "#FFFFCC"; }  else { echo "#F2F2F2";}?> ; border-style:outset; padding: 5px;"><a href="postform.php?NEW_FIELD=YES&mode=EDIT">New Field</a></span> &nbsp; &nbsp; <span style="background-color: <?php  echo "#FFFFCC";?> ; border-style:outset; padding: 5px;"><a href="postlist.php">Job List</a></span> 
...
if ($_REQUEST['save_col']!='') {
	...
	if (is_numeric($_REQUEST['field_id'])) { // its a dynamic field, we can get the details from the database
		...
		$fields = JB_schema_get_static_fields(1);
	...
	if ($field_row['template_tag']=='') { // need to fix the template tag!
		...
		$field_row['template_tag'] = JB_generate_template_tag(1);

		// update form field

		$sql = "UPDATE form_fields SET `template_tag`='".jb_escape_sql($field_row['template_tag'])."' WHERE form_id=1 AND field_id='".jb_escape_sql($_REQUEST['field_id'])."'";
		JB_mysql_query ($sql);
	...
	if ($error=='') {
		...
		$JBMarkup->error_msg('Cannot save due to the following errors:');//echo "<font color='red'></font><br>";
		echo $error;
...
echo '<a href="postlist.php">+ Add new column</a>';
...
?>
	...
	<tr>
		...
		JB_field_select_option_list (1, $col_row['field_id']);
	...
	if ($_REQUEST['column_id']=='') { // get the last sort order
		...
		$sql = "SELECT max(sort_order) FROM form_lists WHERE form_id=1 GROUP BY column_id ";
		$result = JB_mysql_query ($sql) or die (mysql_error().$sql);
	...
	<tr>
		<td>Admin Only?</td>
		<td> <input <?php if ($col_row['admin']!='Y') echo ' checked '; ?> type="radio" name="admin_only" value='N'>No  / <input <?php if ($col_row['admin']=='Y') echo ' checked '; ?> type="radio" name="admin_only" value='Y'> Yes - show only in Admin list
...
<hr>
Here are the columns that will appear on the job list:
<table id='joblist' cellspacing="1" cellpadding="5" style="margin: 0 auto; background-color: #d9d9d9; width:99%; border:0px" >
	<?php

	JB_echo_list_head_data_admin(1);
